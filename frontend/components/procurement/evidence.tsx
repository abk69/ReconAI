...
export function EvidencePanel({ evidence }: { evidence: Record<string, unknown> }) {
  ...
  return (
    <section className="rounded-md border border-line bg-surface p-5 shadow-card">
      <h2 className="text-base font-semibold text-ink">Deterministic reconciliation evidence</h2>
      <p className="mt-1 text-sm leading-6 text-ink-muted">
        Stored by the reconciliation engine. These values are not recomputed in the browser, and
        they are not an AI explanation.
      </p>
      {scalars.length === 0 ? (
